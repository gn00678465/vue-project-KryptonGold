<template>
  <section class="container">
    <Card :set="CardSet[0]">
      ...
      <template #button>
        <button type="button" class="pure-button pure-button-outline-secondary">選購去</button>
      </template>
    </Card>
    <Card :set="CardSet[1]">
      <template #title>有青才敢大聲！</template>
      <template #content>上帝創造世界花了七天，台灣環島一週須要12天，賽德克票房四天破億，人會再一夜間長大，而這支啤酒必須在18天內喝完。</template>
      <template #button>
        <button type="button" class="pure-button pure-button-outline-light">選購去</button>
      </template>
    </Card>
    <Card :set="CardSet[2]">
      <template #title>是檸檬，我加了檸檬!!</template>
      <template #content>臺虎精釀的「臺式檸檬沙瓦Formosawa」將大量檸檬果汁加入這款新品，整體果汁含量高達12%，喝起來充滿檸檬清香與甜味。</template>
      <template #button>
        <button type="button" class="pure-button pure-button-outline-secondary">選購去</button>
      </template>
    </Card>
